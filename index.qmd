...
1. Lower thresholds are more cautious; higher ones accept more risk to enable faster decisions. Hover over the curve to see how `Expected # of trials` decreases with higher thresholds.<br>
...
# Format values row-by-row based on Metric
formatted_df = df.copy()

for idx, row in df.iterrows():
    metric = row["Metric"]
    if metric in ["Projected Mean @ Year-End"]:
    ...
    elif metric in [ "Selected Best Condition"]:
        ...
        formatted_df.at[idx, "Bayesian"] = f"{row['Bayesian']:.2f}"
        formatted_df.at[idx, "SPRT"] = f"{row['SPRT']:.2f}"
        formatted_df.at[idx, "Fixed"] = f"{row['Fixed']:.2f}"
