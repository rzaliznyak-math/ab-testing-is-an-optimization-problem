...
Significance-based methods — SPRT and Fixed — were built to detect big effects. But in our simulation, the average treatment effect is 100. That means most real-world gains fall below their detection threshold.

**SPRT** is tuned to test MDE (101) vs. 100. So when the effect is 100.3, it shrugs and moves on.<br>
**Fixed** tests are underpowered for anything below the MDE = 101, making them blind to most improvements.

The result is that **Significance** methods routinely miss marginal winners and leave real value on the table.
**Bayesian Testing** looks at risk and acts when the math says it’s safe.

Across `{python} f"{NUMBER_SIMULATIONS:,}"` simulations, Bayesian testing delivered 2x the marginal value of significance testing.
