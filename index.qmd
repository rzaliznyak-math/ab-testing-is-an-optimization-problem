...
If you design, run, or analyze AB tests to drive business outcomes — whether in product, growth, or marketing — this paper is for you. It’s a warning about the hidden costs of using significance testing.

Significance testing, as typically applied in industry, is misaligned with how businesses actually create value. It was built for science — where false positives are catastrophic, and delays are acceptable. But in the business world, the opposite is often true: **the real risk isn’t acting too soon — it’s waiting too long.**

This paper explores what happens when we let go of arbitrary defaults — like α = 0.05, 80% power, and fixed MDEs — and focus instead on decision quality. Using Bayesian inference and large-scale simulations, I’ll show how that shift leads to faster tests, more accurate decisions, and more profitable experimentation.

> You run a test. The variant improves conversion by 0.7%. It’s not statistically significant — so you kill it. What did you just leave on the table — and how many times has that happened before?
...
> What if you throw away 20 experiments in a year each with p-values just above 0.05. How many of them were real effects?!
...
In this paper we use expected loss as our stopping criterion — a measure that quantifies the risk of making the wrong decision, combining both the likelihood of harm and its expected magnitude:
...
The key advantage of this framework is that we know the true effect coded into each experiment, while the analysis methods (Bayesian and Significance) operate blindly, equally vulnerable to noise and randomness. Only after all decisions are made do we compare their outcomes — measuring accuracy, timing, and business value.

## Experiment Design

Each of the `{python} f"{NUMBER_SIMULATIONS:,}"` simulated experiments runs for up to 20 days, with traffic split 50/50 between control and treatment.
...
Even when a treatment is truly better (i.e., ITC > 100), it can underperform in a given simulation due to random variation — just as it might in the real world.

```{python}
#| echo: false
#| code-fold: true
#| fig-cap: "Normal Distribution with μ = 100 and σ = 0.5."
from numpy.random import laplace, normal, seed
...
To explore tradeoffs between speed and precision, we simulate decisions across a range of candidate thresholds — e.g., (from 0.01% to 0.10%).
...
optimal_potential = potentials[optimal_index]
optimal_run_time = times[optimal_index]

expedited_risk_level = 4*optimal_risk_level
expedited_index = argmin(abs(array(threshold_values) - expedited_risk_level))
expedited_potential = potentials[expedited_index]
expedited_run_time = times[expedited_index]
...
threshold_figure.add_trace(
  go.Scatter(
      x=[expedited_risk_level],
      y=[expedited_potential],
      mode="markers",
      showlegend = True,
      marker=dict(
    size=16,
    color="orange",  # soft green
    symbol="circle-dot"
)
,
      # name=f"Experiment {'Maximum' if metric_direction == 'increase' else 'Minimum'}",
...
# Example data
df = pd.DataFrame({
    "Threshold": [optimal_risk_level, expedited_risk_level],
    "Projected Mean": [optimal_potential, expedited_potential],
    "Expected Trials": [optimal_run_time, expedited_run_time]
}, index=["optimal", "expedited"])

# First row runtime
first_runtime = df["Expected Trials"].iloc[0]

# Time Savings vs first row (empty for first)
df["Time Savings"] = [
    "" if i == 0 else (first_runtime - val) / first_runtime
    for i, val in enumerate(df["Expected Trials"])
]

# More Experiments (empty for first)
df["More Experiments"] = [
    "" if i == 0 else (first_runtime / val) - 1
    for i, val in enumerate(df["Expected Trials"])
]

# Format columns
df["Threshold"] = df["Threshold"].apply(lambda x: f"{x:.3%}")
df["Projected Mean"] = df["Projected Mean"].apply(lambda x: f"{x:.3%}")
df["Expected Trials"] = df["Expected Trials"].apply(lambda x: f"{x:,.0f}")
df["Time Savings"] = df["Time Savings"].apply(lambda x: f"{x:.2%}" if x != "" else "")
df["More Experiments"] = df["More Experiments"].apply(lambda x: f"{x:.2%}" if x != "" else "")


df = df.drop(columns=["More Experiments"])
...
<p>
 ...
 - Bayesian Test Design allows me to choose any Risk Threshold and know exactly how it affects my expected value.
 - In this case, we can take on 4x the optimal level of risk and we barely compromise our `Projected Mean @ year-end`.

We might choose **Expedited** over **Optimal** because the experiment will run faster. Each subsequent experiments brings in its own value.
...
3. It's possible to average high values of `Marginl Events vs No Test` even if your mean of treatment effects is null.


## Bayesian Vs. Significance

Let’s see how the same `{python} f"{NUMBER_SIMULATIONS:,}"` simulations perform under traditional significance testing. We’ll use a standard setup: α = 0.05, 80% power, and a minimum detectable effect of 101.
...
#| echo: false
#| output: true
#| fig-cap: "With true effects centered at 100; SPRT and Fixed rarely detect the best condition."
...
df = pd.DataFrame({
    ...
    "Bayes (Expedited)": [
        potentials[expedited_index],
        winners[expedited_index],
        times[expedited_index] / number_daily_visitors
    ],
    "SPRT": [
...
# --- Marginal Events vs No Test (per approach) ---
...
bayes_optimal_marginal = max_volume_year * (
    potentials[optimal_index] - control_mean
)
bayes_expedited_marginal = max_volume_year * (
    potentials[expedited_index] - control_mean
)

# Append marginal row
df.loc[len(df.index)] = [
    "Marginal Events vs No Test",
    bayes_optimal_marginal,
    bayes_expedited_marginal,
    sprt_marginal,
    fixed_marginal
]
...
for idx, row in df.iterrows():
    metric = row["Metric"]
    if metric in ["Selected Best Condition"]:
        formatted_df.at[idx, "Bayes (Optimal)"] = f"{row['Bayes (Optimal)']:.2%}"
        formatted_df.at[idx, "Bayes (Expedited)"] = f"{row['Bayes (Expedited)']:.2%}"
        formatted_df.at[idx, "SPRT"] = f"{row['SPRT']:.2%}"
        formatted_df.at[idx, "Fixed"] = f"{row['Fixed']:.2%}"

    elif metric in ["Projected Mean @ Year-End"]:
        formatted_df.at[idx, "Bayes (Optimal)"] = f"{row['Bayes (Optimal)']:.3%}"
        formatted_df.at[idx, "Bayes (Expedited)"] = f"{row['Bayes (Expedited)']:.3%}"
        formatted_df.at[idx, "SPRT"] = f"{row['SPRT']:.3%}"
        formatted_df.at[idx, "Fixed"] = f"{row['Fixed']:.3%}"

    elif metric == "Marginal Events vs No Test":
        formatted_df.at[idx, "Bayes (Optimal)"] = f"{row['Bayes (Optimal)']:,.0f}"
        formatted_df.at[idx, "Bayes (Expedited)"] = f"{row['Bayes (Expedited)']:,.0f}"
        formatted_df.at[idx, "SPRT"] = f"{row['SPRT']:,.0f}"
        formatted_df.at[idx, "Fixed"] = f"{row['Fixed']:,.0f}"

    else:  # Mean Run Time
        bayes_opt_raw = row["Bayes (Optimal)"] * number_daily_visitors
        bayes_exp_raw = row["Bayes (Expedited)"] * number_daily_visitors
        sprt_raw = row["SPRT"] * number_daily_visitors
        fixed_raw = row["Fixed"] * number_daily_visitors

        formatted_df.at[idx, "Bayes (Optimal)"] = f"N = {bayes_opt_raw:,.0f} ({row['Bayes (Optimal)']:.2f} days)"
        formatted_df.at[idx, "Bayes (Expedited)"] = f"N = {bayes_exp_raw:,.0f} ({row['Bayes (Expedited)']:.2f} days)"
        formatted_df.at[idx, "SPRT"] = f"N = {sprt_raw:,.0f} ({row['SPRT']:.2f} days)"
        formatted_df.at[idx, "Fixed"] = f"N = {fixed_raw:,.0f} ({row['Fixed']:,.0f} days)"

formatted_df = formatted_df.drop(columns=["Bayes (Expedited)"])
# Display without index
...
Traditional significance tests — SPRT and Fixed — are built to detect big, obvious wins. But in these simulations, the average effect is 100. Small gains like 100.3? SPRT shrugs. Fixed tests, tuned to detect 101, don't have time to notice.

These methods aren’t wrong — they’re just answering the wrong question. They optimize for avoiding false positives, not for capturing real value. As the comparison shows, Significance Tests routinely miss marginal winners and leaves potential gains on the table.

**Bayesian testing**, on the other hand, isn’t concerned with p-values, power, or minimum detectable effects. It focuses on a single question: Is the risk low enough to act?
That mindset proves far more profitable — across `{python} f"{NUMBER_SIMULATIONS:,}"` simulations, Bayesian testing delivered **2x the marginal value** of Significance testing.
...
hypothesis testing/significance testing is predisposed to retain the null, even in the face of contrary evidence.
It's possible to make money even we average a Null effect!
-->

# Counterargument

Some might argue that this comparison was unfair — that we "stacked the deck" against significance testing by choosing α = 0.05, 80% power, and an MDE of 101.

But those aren’t cherry-picked values — they’re the defaults used across much of the tech industry. If those defaults lead to missed wins and underpowered tests, that’s not a critique of this simulation — it’s a critique of the status quo.

Others might say significance testing could perform better with a different combo of α, β, and MDE. But if you're tuning those parameters to maximize business impact...
You're designing a Bayesian AB Test — just with extra steps.

# Conclusion

Significance testing has its place, but in business experimentation where time and resources are finite, it’s too cautious for its own good. Designed to minimize false positives, it delays action even when the downside is minimal. Valuable wins are discarded in the name of statistical purity.

Bayesian testing is less risk averse. It doesn’t demand overwhelming proof — just enough evidence to make a smart, low-risk decision. That mindset enables faster tests and more accurate decisions that generate way more value.

This isn’t limited to conversion rates. The same approach works for continuous metrics like revenue, or for metrics you want to reduce — like churn, cancellations, or latency.

If you design, run, or analyze experiments, ask yourself: Is your testing framework helping you act — or holding you back?

I’d love to connect — reach out to ask questions or to learn how to run these simulations on your own; contact via [email](mailto:rzaliznyak@gmail.com) or on LinkedIn.