...
This paper explores what happens when we let go of inherited defaults — like α = 0.05, 80% power, and fixed MDEs — and focus instead on decision quality. Using Bayesian inference and large-scale simulations, I’ll show how that shift leads to faster tests, more accurate decisions, and more profitable experimentation.
...
> What if you throw away 20 experiments in a year each with p-values just above 0.05. How much could you be sacrificing?
...
3. It's possible to average many `Marginl Events vs No Test` even if your distribution of treatment effects null mean of 0.
...
Significance testing has its place — but in business experimentation, it’s often too cautious for its own good. Designed to minimize false positives, it delays action even when the downside is minimal. Valuable wins are discarded in the name of statistical purity.

Bayesian testing takes a different path. It doesn’t demand overwhelming proof — just enough evidence to make a smart, low-risk decision. That mindset enables faster tests and more accurate decisions.

And this isn’t limited to conversion rates. The same approach works for continuous metrics like revenue, or for metrics you want to reduce — like churn, cancellations, or latency.
