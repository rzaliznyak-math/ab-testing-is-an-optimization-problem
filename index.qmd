...
Bayesian testing isn’t a new set of formulas — it’s a philosophical shift. One that aligns probabilistic thinking with how businesses actually need to make decisions.
...
Going forward we refer to _expected loss_ as `Risk`. We will use `Risk` to stop our experiments and seek to determine the optimal level of `Risk` to bear for any given experiment design.
...
df = pd.DataFrame({
    "Metric": ["Projected Mean @ Year-End", "Selected Best Condition", "Mean Run Time"],
    "Bayesian": [potentials[optimal_index],winners[optimal_index],times[optimal_index]/number_daily_visitors],
    "SPRT": [
...
bayes_marginal = max_volume_year * (potentials[optimal_index] - control_mean)
...
display(df.style.hide(axis="index"))  # Requires Pandas ≥1.4.0
```
